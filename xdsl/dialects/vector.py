...
class Broadcast(IRDLOperation):
    ...
    source = OperandDef(AnyAttr())
    vector = ResultDef(VectorType)
...
class FMA(IRDLOperation):
    name = "vector.fma"
    lhs = OperandDef(VectorType)
    rhs = OperandDef(VectorType)
    acc = OperandDef(VectorType)
    res = ResultDef(VectorType)
...
class Maskedload(IRDLOperation):
    ...
    memref = OperandDef(MemRefType)
    indices = VarOperandDef(IndexType)
    mask = OperandDef(VectorBaseTypeAndRankConstraint(i1, 1))
    passthrough = OperandDef(VectorType)
    res = ResultDef(VectorRankConstraint(1))
...
class Maskedstore(IRDLOperation):
    ...
    indices = VarOperandDef(IndexType)
    mask = OperandDef(VectorBaseTypeAndRankConstraint(i1, 1))
    value_to_store = OperandDef(VectorRankConstraint(1))
...
class Createmask(IRDLOperation):
    ...
    mask_operands = VarOperandDef(IndexType)
    mask_vector = ResultDef(VectorBaseTypeConstraint(i1))
